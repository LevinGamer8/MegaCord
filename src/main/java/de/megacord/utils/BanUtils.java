...
public class BanUtils {
    ...
    public BanUtils(String name, String ip, DataSource source, Configuration settings, Configuration standardBans) {
        ...
        try (Connection conn = getSource().getConnection();
            ...
            MegaCord.logger().log(Level.WARNING, "could not fetch ban data from database", e);
        }
    }
    ...
    public CompletableFuture<Void> editban(int type, String value1, String editBy) {
        // TYPE
        // 1 = Ban (Ban oder Mute)
        // 2 = Bis
        // 3 = Grund
        return CompletableFuture.runAsync(() -> {
            ...
            switch (type) {
                case 1:
                    try (Connection conn = getSource().getConnection(); PreparedStatement ps = conn.prepareStatement("UPDATE bannedplayers SET Ban = ?, baneditiertvon = ? WHERE TargetName = ?")) {
                        ...
                        MegaCord.logger().log(Level.WARNING, "could not edit ban to new value ban or mute", e);
                    }
                    break;
                case 2:
                    if (value.equalsIgnoreCase("-1")) {
                        try (Connection conn = getSource().getConnection(); PreparedStatement ps = conn.prepareStatement("UPDATE bannedplayers SET Bis = ?, Perma = ? , baneditiertvon = ? WHERE TargetName = ?")) {
                            ...
                            MegaCord.logger().log(Level.WARNING, "cloud not update ban to new time permanent", e);
                        }
                        break;
                    }
                    ...
                    try (Connection conn = getSource().getConnection(); PreparedStatement ps1 = conn.prepareStatement("UPDATE bannedplayers SET Bis = ?, Perma = ?, baneditiertvon = ? WHERE TargetName = ?")) {
                        ...
                        MegaCord.logger().log(Level.WARNING, "could not update ban to new time", e);
                    }
                    break;
                case 3:
                    try (Connection conn = getSource().getConnection(); PreparedStatement ps = conn.prepareStatement("UPDATE bannedplayers SET Grund = ?, baneditiertvon = ? WHERE TargetName = ?")) {
                        ...
                        MegaCord.logger().log(Level.WARNING, "could not update ban to new reason", e);
                    }
                    break;
            }
            ...
            for (ProxiedPlayer all : ProxyServer.getInstance().getPlayers()) {
                if ((all.hasPermission("megacord.punish.notify") || all.hasPermission("megacord.*")) || all.getUniqueId().toString().equalsIgnoreCase(editBy))
                    all.sendMessage(tc);
            }
        }, MegaCord.getInstance().EXECUTOR_SERVICE);
    }
    ...
    public CompletableFuture<Boolean> unban(boolean msg, String name) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (Connection conn = getSource().getConnection(); PreparedStatement ps = conn.prepareStatement("UPDATE history SET VonEntbannt = ? WHERE TargetName = ?  AND Erstellt = ?")) {
                ...
                MegaCord.logger().log(Level.WARNING, "cannot update VonEntbannt to new value", e);
                return false;
            }
            return true;
        }, MegaCord.getInstance().EXECUTOR_SERVICE);
    ...
    public CompletableFuture<Boolean> isBanned(String name) {
        return CompletableFuture.supplyAsync(() -> {
            try (Connection conn = getSource().getConnection();
                ...
                MegaCord.logger().log(Level.WARNING, "could not check if player is banned", e);
            }
            return false;
        }, MegaCord.getInstance().EXECUTOR_SERVICE);
    }
    ...
    public CompletableFuture<Integer> containsIP() {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (Connection conn = getSource().getConnection();
                ...
                MegaCord.logger().log(Level.WARNING, "cloud not check if database containsip", e);
            }
            return -1;
        }, MegaCord.getInstance().EXECUTOR_SERVICE);
    }
    ...
    public CompletableFuture<Integer> getBanCount(String grund, boolean reason) {
        return CompletableFuture.supplyAsync(() -> {
            try (Connection conn = getSource().getConnection();
                ...
                MegaCord.logger().log(Level.WARNING, "cloud not count how many bans a user have", e);
            }
            return -1;
        }, MegaCord.getInstance().EXECUTOR_SERVICE);
    }
    ...
    public void banByStandard(int standardID, String ip) {
        ...
        try {
            ...
            MegaCord.logger().log(Level.WARNING, "cannot ban user by default cause of invalid format in standardbans.yml", e);
            return;
        }
    ...
    private void checkID(int standardID) {
        ...
        if (!standardBans.get("BanIDs." + standardID + ".Reason").equals(reasons[(standardID - 1)])) {
            ...
            try {
                ConfigurationProvider.getProvider(YamlConfiguration.class).save(standardBans, Config.standardBansFile);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
