...
public class HistoryManager {
    ...
    public void insertInDB(UUID targetUUID, UUID vonUUID, String type, String grund, long erstellt, long bis, int perma, int ban) {
        try (Connection conn = MegaCord.getInstance().getDataSource().getConnection(); PreparedStatement createReport = conn.prepareStatement("INSERT INTO history (TargetUUID,VonUUID,Type,Grund,Erstellt,Bis,Perma,Ban) VALUES(?,?,?,?,?,?,?,?)");) {
    ...
    public List<HistoryElement> readHistory(UUID target, int limit, int page, String what, boolean lastOnes) {
        ...
        try (Connection conn = MegaCord.getInstance().getDataSource().getConnection();
            ...
            while (rs.next()) {
                reports.add(new HistoryElement(UUID.fromString(rs.getString("TargetUUID")), UUID.fromString(rs.getString("VonUUID")), rs.getString("Type"), rs.getString("Grund"), rs.getLong("Erstellt"), rs.getLong("Bis"), rs.getInt("Perma"), rs.getInt("Ban"), rs.getString("VonEntbannt")));
            }
        } catch (SQLException e) {
